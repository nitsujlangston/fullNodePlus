...
TransactionSchema.statics.mintCoins = async function (params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let findStream = updateData.bufferCount(200).flatMap((updatesArr) => {
      return updatesArr.map(async (update) => {
        let wallet = findWallet(update.address);
        let walletData = Object.assign({wallet}, update);
        return walletData;
      });
    }); 

    let mintOps = [];
    let updateStream = findStream.map((data) => {
      let {tx, index, address, satoshis, scriptBuffer, wallets } = data;
