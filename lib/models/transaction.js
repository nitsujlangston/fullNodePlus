...
TransactionSchema.statics.mintCoins = function (params) {
  ...
  let findWallet = (address) => { 
    return WalletAddress.collection.find({address})
  ...
  let findStream = updateData.bufferCount(200)
    .mergeMap((updatesArr) => {
    let updates = updatesArr.map(async(update) => {
      let wallet;
      try {
        wallet = await findWallet(update.address);
      } catch(e) {
  ...
  let mintOps = [];
  let updateStream = findStream
    .mergeMap((bulkUpdates) => {
