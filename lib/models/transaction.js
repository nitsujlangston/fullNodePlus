...
TransactionSchema.statics.addTransactions = async function(params){
  ...
  return new Promise(async (resolve) => {
    let groupedTxids = partition(txs.map((tx) => tx.hash), 500);
    let mintWallets = [];
    let spentWallets = [];
    for(let txids of groupedTxids) {
      let walletBatch = await Coin.collection.aggregate([
        {
      ...
      mintWallets.push(...walletBatch);
      spentWallets.push(...spentBatch);
    }
    let allWallets = mintWallets.concat(spentWallets);
    let allTxWallets = _.groupBy(allWallets, (wallet) => wallet._id);
    let txOps = [];
    for(let tx of txs) {
      let txWallets = allTxWallets[tx.hash] || [];
      let wallets = {};
      ...
      let updateWallets  = partition(Object.values(wallets), 25);
      for(let walletGroup of updateWallets) {
        txOps.push({
          updateOne: {
            filter: {
              txid: tx.hash
