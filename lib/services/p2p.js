        ...
        async.eachOfSeries(self.headersQueue, function(header, headerIndex, cb) {
          ...
          self.getBlock(header.hash, function(err, block) {
            Block.addBlock(block, function(err) {
               ...
               *});
               */
              //let time = new Date(block.header.time * 1000).toISOString();
              let blockImport = {time: block.header.time, curHeight: bestBlock.height, totalHeight: self.getPoolHeight()} ;
              LogEvents.blockImport.next(blockImport);
              cb(err);
            });
          });
        }, function(err) {
