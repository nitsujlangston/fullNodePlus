...
LogEvents.blockImport
  ...
  .subscribe((update) => {
    ...
    let downMs = Math.floor(nowDate - startTime).toFixed(0);
    let downTime =  (downMs/1000 - now).toFixed(0);
    ...
    let avgBlockRate = (tick + downTime) / (blockCount * tick);
    ...
    console.log(`Syncing: ${ blocks } blocks, ${ now } s, downtime ${ downTime }s, ${ blockRate } s/block ${ blocksEta } hrs ${ blockTime } `);
